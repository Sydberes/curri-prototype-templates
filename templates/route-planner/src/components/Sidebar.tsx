...
export type ActiveNav = 'Overview' | 'Planner' | 'Exceptions'

const TOP = [
  { label: 'Book delivery', icon: Lightning },
  { label: 'Delivery history', icon: ClockCounterClockwise },
  { label: 'Drafts', icon: NotePencil },
]
...
export function Sidebar({
  ...
  active: ActiveNav
  onSelect: (a: ActiveNav) => void
}) {
  const ROUTE_PLANNER: Item[] = [
    ...
    { label: 'Request a route', icon: Plus },
    { label: 'Status', icon: CalendarCheck },
    {
      label: 'Exceptions',
      icon: WarningCircle,
      badge: 6,
      active: active === 'Exceptions',
      onClick: () => onSelect('Exceptions'),
    },
    { label: 'Route reviews', icon: ChartLineUp },
...
function NavItem({ label, icon: Icon, active, badge, onClick }: Item) {
  return (
    <button
      type="button"
      onClick={onClick}
